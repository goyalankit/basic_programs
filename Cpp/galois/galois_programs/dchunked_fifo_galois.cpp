...
struct AsyncAlgo {
	...
	void operator()(GNode& source) const {
		...
		Galois::for_each< dChunk >(initial.begin(), initial.end(), *this);
	}
...
int main(int argc, char **argv){
	...
	std::cout << "Time taken by parallel galois dchunked fifo implementation on " << graph.size() << " nodes is " <<  (((end.tv_sec  - start.tv_sec) * 1000000u +  end.tv_usec - start.tv_usec) / 1.e6) << std::endl;
